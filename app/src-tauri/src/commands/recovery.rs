...
fn row_to_relapse_entry(row: &rusqlite::Row) -> rusqlite::Result<RelapseEntry> {
    Ok(RelapseEntry {
        ...
        activity_before: row.get("activity_before")?,
        emotional_state: row.get("emotional_state")?,
        resistance_technique: row.get("resistance_technique")?,
        urge_intensity: row.get("urge_intensity")?,
        notes: row.get("notes")?,
        urge_entry_id: row.get("urge_entry_id")?,
